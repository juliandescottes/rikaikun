...
*/
	...
	tryUpdatePopup: function(ev) {
		...
		var rp, ro;
		// Firefox supports caretPositionFromPoint, while WebKit and Edge support caretRangeFromPoint
		if (typeof document.caretPositionFromPoint != "undefined") {
				start = document.caretPositionFromPoint(ev.clientX, ev.clientY);
				rp = start.offsetNode;
				ro = start.offset;
		} else if (typeof document.caretRangeFromPoint != "undefined") {
				start = document.caretRangeFromPoint(ev.clientX, ev.clientY);
				rp = start.startContainer;
				ro = start.startOffset;
		}
